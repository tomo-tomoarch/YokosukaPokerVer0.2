...
public class GameControler : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        //middleRankSender = GetComponent<MiddleRankSender>();
    }
    private void Update()
    {
    ...
    void OnGUI()
    {
        if (rankGetButton.ToggleReadyMiddle())
        {
            if (GUI.Button(new Rect(buttonX, 250, 100, 28), "check hand!!"))
            {
 
                int rank = (int)PhotonNetwork.room.customProperties[PhotonNetwork.player.ID + "middleRankhand0"];
                int rankB = (int)PhotonNetwork.room.customProperties[PhotonNetwork.player.ID + "middleRankhand1"];
                int rankC = (int)PhotonNetwork.room.customProperties[PhotonNetwork.player.ID + "middleRankhand2"];
                int rankD = (int)PhotonNetwork.room.customProperties[PhotonNetwork.player.ID + "middleRankhand3"];
                int rankE = (int)PhotonNetwork.room.customProperties[PhotonNetwork.player.ID + "middleRankhand4"];

                handRank = new int[5] { rank, rankB, rankC, rankD, rankE }; //リストの作成

                rankGetButton.middle = false;
    ...
    void sendhandrank(double i)
    {
        ...
        properties.Add(PhotonNetwork.player.ID + "middle", i);
